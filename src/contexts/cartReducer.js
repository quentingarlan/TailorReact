...
export const sumItems = cartItems => {
    ...
    let total = cartItems.reduce((total, product) => total + shipping + product.price * product.quantity, 0).toFixed(2);
    ...
    switch (action.type) {
        ...
        case "INCREASE":
            state.cartItems[state.cartItems.findIndex(item => item.clothImageName === action.payload.clothImageName
                ...
                && item.thighSize === action.payload.thighSize
                && item.length === action.payload.length)].quantity++
            return {
                ...state,
                ...sumItems(state.cartItems),
                cartItems: [...state.cartItems]
